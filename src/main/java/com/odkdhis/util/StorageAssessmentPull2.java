 ...
 */
@Component
public class StorageAssessmentPull2 extends CommonUtil {
    ...
    @Scheduled(cron ="10 19 * * * ?", zone = "Africa/Nairobi")
    ...
    private void basicStorageData(String formId) throws Exception {
        ...
        for (int i = 0; i < jArray.length(); i++) {
            ...
            StorageAssessmentModel storage = new StorageAssessmentModel.storageAssessmentsBuilder().instanceId(instanceId)
                    ...
                    .orgUnit(orgUnit).storeCleaning(storeCleaning).arrangement(arrangement).dSunlight(dSunlight).vermin(vermin).shelvelPallets(shelvelPallets).lighting(lighting)
