 ...
 */
@Controller
public class SOhExpectedPerFacilityController extends CommonUtil {
    
    @GetMapping(value = {"/sohexpectedperfacility"})
    public ModelAndView dashboard() throws ParseException {
        ...
        modelAndView.addObject("sohexpectedperfacilitysearch", new FacilitiesMeetingSohExpectation());
        modelAndView.addObject("sohexpectedperfacilitydata", accountabilityDao.getSohExpectedDataPerFacility(county, dateFrom, dateTo));
        //modelAndView.addObject("summaryOrgUnit",  "NOT PROVIDED" );
        modelAndView.addObject("summaryOrgUnit", "% of HFs with actual soh=expected soh : " + county +" Between "+format.format(dateFrom)+ " and "+format.format(dateTo));
        modelAndView.setViewName("sohexpectedperfacility");
        return modelAndView;
    }

    
        @GetMapping(value = {"/sohexpectedperfacilityfilter"})
    public ModelAndView dashboardfiltered(@RequestParam(name = "county", required = false) String county, @RequestParam(name = "fromDate", required = false) String fromDate, @RequestParam(name = "toDate", required = false) String toDate) throws ParseException {
        ...
        List<FacilitiesMeetingSohExpectation> lst = new ArrayList<>();
        if (county != null && fromDate != null && toDate != null) {
            lst = accountabilityDao.getSohExpectedDataPerFacility(county, dateFrom, dateTo);
        }
        
        ModelAndView modelAndView = getModelAndView();
        modelAndView.addObject("sohexpectedperfacilitysearch", new FacilitiesMeetingSohExpectation());
        modelAndView.addObject("sohexpectedperfacilitydata", lst);
        modelAndView.addObject("summaryOrgUnit", "% of HFs with actual soh=expected soh : " + county +" Between "+format.format(dateFrom)+ " and "+format.format(dateTo));
